...
/*! Initialize #AKMPRMS structure and make APIs ready to use. This function
  ...
  @param[in] apat Specify a layout pattern number.  The number is determined
  according to the mount orientation of the accelerometer.
  ...
  @param[in] magCodePerUt Specify the magnetometer sensitivity, code/uT
  @param[in] accCodePerG Specify the accelerometer sensitivity, code/g
*/
int16 AKFS_Init(
		void		*mem,
		const	AKFS_PATNO	hpat,
		const	AKFS_PATNO	apat,
		const	uint8		regs[],
		const   AKFLOAT	        accOffset[],
		const   AKFLOAT         magCodePerUt,
		...
		)
{
  ...
  /* Sensitivity */
  prms->fv_hs.u.x = magCodePerUt;
  prms->fv_hs.u.y = magCodePerUt;
  prms->fv_hs.u.z = magCodePerUt;
...
{
  AKMPRMS *prms __attribute__((unused));
#ifdef AKM_VALUE_CHECK
  if (mem == NULL) {
    AKMDEBUG(AKMDATA_CHECK, "%s: Invalid mem pointer.", __FUNCTION__);
    return AKM_ERROR;
